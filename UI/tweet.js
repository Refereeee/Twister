import { HeaderView, Tweet, TweetFeed, tweets } from './modules/modules.js';
//
// TweetFeedView,HeaderView,TweetFeed,
...
class TweetView {
  ...
  static commentTime(el) {
    if ((Date.now() - el.createdAt) < 86400000) {
      ...
      return `${TweetView.month[el.createdAt.getMonth()]} ${el.createdAt.getHours()}h.  | `;
    }
  }
  ...
  static postTime(el) {
    return `${el.createdAt.getHours()}:${TweetView.postMinutes(el)} ${TweetView.getAmPm(el)} | ${TweetView.month[el.createdAt.getMonth()]} ${el.createdAt.getDate()}, ${el.createdAt.getFullYear()}`;
  }

  showTweet(id) {
    if(!TweetView.validateId(id)) {
      throw new Error('Validate id error');
    }
    return this.tweetLoad = this.list.find(el => el.id === `${id}`);
  ...
  display() {
      ...
      `<div class="twit__wrapper">
              <div class="posts__public">
                  <div class="posts__user_img">
                    <img src="./imgs/${TweetView.pseudo(this.tweetLoad.author) + '.png'}" alt="picture" class="posts__add_picture">
                  </div>
                  <div class="posts__user">
                      <h3>${this.tweetLoad.author}</h3>
                      <span class="posts__psevdo">${'@' + TweetView.pseudo(this.tweetLoad.author)}</span>
                    ...
                    <div class="posts__content">
                          <span class="posts_time">${TweetView.postTime(this.tweetLoad)}</span>
                          <article>${this.tweetLoad.text}</article>
                      <div class="posts__comment">
                           ...
                           <span class="posts__comment_number">${this.tweetLoad.comments.length}</span>
                      </div>
                    </div>
                  </div>
              </div>
    ...
    this.tweetLoad.comments.forEach(el=>{
      commentsHtml.insertAdjacentHTML('afterbegin',
        `<div class="twit__comment">
                    <div class="posts__user_img"><img src="./imgs/${TweetView.pseudo(el.author) + '.png'}" alt="picture" class="posts__add_picture">
                    </div>
                    <div class="posts__user">
                        <h3>${el.author}</h3>
                        <span class="posts__psevdo">${'@' + TweetView.pseudo(el.author)}</span>
                        <div class="posts__content">
...
// const TweetShow = new TweetView('#tweet');


const head = new HeaderView('#container__user');
const newTf = new TweetFeed();
const tweetShow = new TweetView('#tweet');
tweetShow.showTweet('13')


class Global {
  constructor(arr) {
    this.arrHeadTweet = arr
  }

  setCurrentUser(name) {
    (this.arrHeadTweet)[0].forEach(el => {
      el.setCurrentUser(name);
    });
  }
  showTweet(id){
    (this.arrHeadTweet)[1][1].showTweet((this.arrHeadTweet)[1][0].getArr(id))
  }


}

const newGlobal = new Global([[head,newTf], [newTf,tweetShow]]);
