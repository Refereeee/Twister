import {HeaderView,TweetFeed,Tweet,Comment,tweets} from './modules/modules.js';
//
...
class TweetView {
  ...
  display() {
    const tweetContainer = document.querySelector(this.containerId);
     const tweetLoad = this.showTweet(id);
    tweetContainer.insertAdjacentHTML('afterbegin',
      `<div class="twit__wrapper">
              <div class="posts__public">
                  <div class="posts__user_img">
                    <img src="./imgs/${TweetView.pseudo(viewTweet.author) + '.png'}" alt="picture" class="posts__add_picture">
                  </div>
                  <div class="posts__user">
                      <h3>${tweetLoad.author}</h3>
                      <span class="posts__psevdo">${'@' + TweetView.pseudo(tweetLoad.author)}</span>
                    ...
                    <div class="posts__content">
                          <span class="posts_time">${TweetView.postTime(tweetLoad)}</span>
                          <article>${tweetLoad.text}</article>
                      <div class="posts__comment">
                           ...
                           <span class="posts__comment_number">${tweetLoad.comments.length}</span>
                      </div>
                    </div>
                  </div>
              </div>
